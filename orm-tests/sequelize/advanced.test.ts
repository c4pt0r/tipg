...
describe('Sequelize Advanced SQL Features [pg-tikv]', () => {
  ...
  beforeAll(async () => {
    ...
    await sequelize.query(`DROP TABLE IF EXISTS adv_sales CASCADE`);
    await sequelize.query(`DROP TABLE IF EXISTS adv_employees CASCADE`);
    await sequelize.query(`DROP TABLE IF EXISTS adv_departments CASCADE`);
    await sequelize.query(`DROP VIEW IF EXISTS adv_dept_summary`);
  ...
  beforeEach(async () => {
    await sequelize.query(`DELETE FROM adv_sales`);
    await sequelize.query(`DELETE FROM adv_employees`);
    await sequelize.query(`DELETE FROM adv_departments`);
  ...
  describe('window functions', () => {
    it('should support ROW_NUMBER()', async () => {
      const result = await sequelize.query<{ name: string; salary: string; rank: string }>(
        `SELECT name, salary, ROW_NUMBER() OVER (ORDER BY salary DESC) as rank
         FROM adv_employees ORDER BY rank`,
    ...
    it('should support RANK() with ties', async () => {
      await sequelize.query(`UPDATE adv_employees SET salary = 70000 WHERE name = 'Charlie'`);

      const result = await sequelize.query<{ name: string; rank: string }>(
        `SELECT name, salary, RANK() OVER (ORDER BY salary DESC) as rank
         FROM adv_employees ORDER BY rank, name`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support DENSE_RANK() with PARTITION BY', async () => {
      const result = await sequelize.query<{ name: string; department_id: number; dept_rank: string }>(
        `SELECT name, department_id, salary,
                DENSE_RANK() OVER (PARTITION BY department_id ORDER BY salary DESC) as dept_rank
         FROM adv_employees ORDER BY department_id, dept_rank`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support LEAD() and LAG()', async () => {
      const result = await sequelize.query<{ name: string; prev_salary: string | null; next_salary: string | null }>(
        `SELECT name, salary,
                LAG(salary) OVER (ORDER BY salary) as prev_salary,
                LEAD(salary) OVER (ORDER BY salary) as next_salary
         FROM adv_employees ORDER BY salary`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support SUM() OVER partition total', async () => {
      const result = await sequelize.query<{ name: string; department_id: number; dept_total: string }>(
        `SELECT name, department_id, salary,
                SUM(salary) OVER (PARTITION BY department_id) as dept_total
         FROM adv_employees ORDER BY department_id, name`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support AVG() OVER', async () => {
      const result = await sequelize.query<{ dept_avg: string }>(
        `SELECT name, department_id, salary,
                AVG(salary) OVER (PARTITION BY department_id) as dept_avg
         FROM adv_employees WHERE department_id = 1`,
        { type: QueryTypes.SELECT }
      );
  ...
  describe('CTE (WITH clause)', () => {
    it('should support simple CTE', async () => {
      const result = await sequelize.query<{ name: string; salary: string }>(
    ...
    it('should support multiple CTEs', async () => {
      const result = await sequelize.query<{ name: string; total: string }>(
        `WITH dept_totals AS (
           SELECT department_id, SUM(salary) as total
           FROM adv_employees GROUP BY department_id
         ),
  ...
  describe('recursive CTE', () => {
    it('should support recursive CTE for hierarchy', async () => {
      const result = await sequelize.query<{ id: number; name: string; level: number }>(
        `WITH RECURSIVE emp_hierarchy AS (
           SELECT id, name, manager_id, 1 as level
           FROM adv_employees WHERE manager_id IS NULL
           UNION ALL
  ...
  describe('subqueries', () => {
    it('should support subquery in WHERE with IN', async () => {
      const result = await sequelize.query<{ name: string }>(
        `SELECT name, salary FROM adv_employees
    ...
    it('should support subquery with EXISTS', async () => {
      const result = await sequelize.query<{ name: string }>(
        `SELECT name FROM adv_employees e
         WHERE EXISTS (
    ...
    it('should support scalar subquery in SELECT', async () => {
      const result = await sequelize.query<{ name: string; sale_count: string }>(
        `SELECT name,
    ...
    it('should support derived table', async () => {
      const result = await sequelize.query<{ dept_name: string; employee_count: string }>(
  ...
  describe('views', () => {
    it('should create and query a view', async () => {
      ...
      const result = await sequelize.query<{ dept_name: string; total_salary: string }>(
  ...
  describe('JSONB operations', () => {
    it('should extract JSONB field with ->', async () => {
      const result = await sequelize.query<{ name: string; level: string }>(
        `SELECT name, metadata->'level' as level
         FROM adv_employees WHERE metadata IS NOT NULL ORDER BY name`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should extract text with ->>', async () => {
      const result = await sequelize.query<{ name: string }>(
        `SELECT name, metadata->>'level' as level
         FROM adv_employees WHERE metadata->>'level' = 'senior' ORDER BY name`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should use JSONB containment @>', async () => {
      const result = await sequelize.query<{ name: string }>(
        `SELECT name FROM adv_employees
         WHERE metadata @> '{"level": "senior"}' ORDER BY name`,
        { type: QueryTypes.SELECT }
      );
  ...
  describe('string functions', () => {
    it('should support UPPER and LOWER', async () => {
      const result = await sequelize.query<{ upper_name: string; lower_email: string }>(
        `SELECT UPPER(name) as upper_name, LOWER(email) as lower_email
         FROM adv_employees WHERE name = 'Alice'`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support CONCAT', async () => {
      const result = await sequelize.query<{ full_info: string }>(
    ...
    it('should support SUBSTRING', async () => {
      const result = await sequelize.query<{ prefix: string }>(
    ...
    it('should support SPLIT_PART', async () => {
      const result = await sequelize.query<{ username: string }>(
  ...
  describe('date/time functions', () => {
    it('should support DATE_TRUNC', async () => {
      const result = await sequelize.query<{ month: Date; total: string }>(
    ...
    it('should support EXTRACT', async () => {
      const result = await sequelize.query<{ month: string; count: string }>(
  ...
  describe('aggregate with HAVING', () => {
    it('should filter groups with HAVING', async () => {
      const result = await sequelize.query<{ department_id: number; emp_count: string }>(
  ...
  describe('DISTINCT ON', () => {
    it('should support DISTINCT ON', async () => {
      const result = await sequelize.query<{ department_id: number; name: string }>(
  ...
  describe('CASE expressions', () => {
    it('should support simple CASE', async () => {
      const result = await sequelize.query<{ name: string; salary_level: string }>(
        `SELECT name,
                CASE
                  ...
                  ELSE 'low'
                END as salary_level
         FROM adv_employees ORDER BY salary DESC`,
        { type: QueryTypes.SELECT }
      );
  ...
  describe('COALESCE and NULLIF', () => {
    it('should support COALESCE', async () => {
      await sequelize.query(`UPDATE adv_employees SET email = NULL WHERE name = 'Alice'`);
    ...
    it('should support NULLIF', async () => {
      const result = await sequelize.query<{ null_result: number | null; not_null_result: number }>(
  ...
  describe('math functions', () => {
    it('should support ABS, CEIL, FLOOR, ROUND', async () => {
      const result = await sequelize.query<{ abs_val: string; ceil_val: string; floor_val: string }>(
        `SELECT ABS(-5) as abs_val, CEIL(4.2) as ceil_val,
                FLOOR(4.8) as floor_val, ROUND(4.567, 2) as round_val`,
        { type: QueryTypes.SELECT }
      );
    ...
    it('should support SQRT, POWER, MOD', async () => {
      const result = await sequelize.query<{ sqrt_val: string; power_val: string; mod_val: string }>(
