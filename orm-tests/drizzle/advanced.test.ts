...
describe('Drizzle Advanced SQL Features [pg-tikv]', () => {
  ...
  beforeAll(async () => {
    ...
    await db.execute(sql`DROP TABLE IF EXISTS adv_sales CASCADE`);
    await db.execute(sql`DROP TABLE IF EXISTS adv_employees CASCADE`);
    await db.execute(sql`DROP TABLE IF EXISTS adv_departments CASCADE`);
    await db.execute(sql`DROP VIEW IF EXISTS adv_dept_summary`);
  ...
  beforeEach(async () => {
    await db.execute(sql`DELETE FROM adv_sales`);
    await db.execute(sql`DELETE FROM adv_employees`);
    await db.execute(sql`DELETE FROM adv_departments`);
  ...
  describe('window functions', () => {
    ...
    it('should support RANK() with ties', async () => {
      await db.execute(sql`UPDATE adv_employees SET salary = 70000 WHERE name = 'Charlie'`);
  ...
  describe('views', () => {
    it('should create and query a view', async () => {
      ...
      expect(result.rows).toHaveLength(3);
      expect(result.rows[0].dept_name).toBe('Engineering');
    });
  });
  ...
  describe('COALESCE and NULLIF', () => {
    it('should support COALESCE', async () => {
      await db.execute(sql`UPDATE adv_employees SET email = NULL WHERE name = 'Alice'`);
